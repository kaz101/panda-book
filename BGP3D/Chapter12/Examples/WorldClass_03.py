...
from PreloaderClass_02 import Preloader
...
class World:
	def __init__(self):
		...
		# Loads the 2D image and polygon that will be used to replace
		# the default windows mouse cursor in game.
		cursor.reparentTo(render2d)
		# Parents the cursor to the render2d node. render2d is used instead
		# of aspect2d because using aspect2d would result in the cursor being unable
		# to reach the farther edges of the window.
		cursor.setBin("gui-popup", 100)
		# The cursor is added to the gui-popup bin to ensure that it is rendered on top
		# of everything else on the screen.		
		cursor.hide()
		# By default, the cursor will not be visible.
		
		
		props = WindowProperties()
		# Creates a WindowProperties object, which stores a wide variety of window settings.
		props.setCursorHidden(True)
		# Sets the WindowProperties object to hide the default mouse cursor.
		base.win.requestProperties(props)
		# Tells the default window to use the altered properties in the WindowProperties object.
		base.mouseWatcherNode.setGeometry(cursor.node())
		# Attaches the loaded custom cursor to the mouse so that will follow mouse movement. Note
		# that this behavior is frame rate dependant, and low frame rates could cause problems with it.
